...
# dfs 함수 내 반복문 수정:
# 현재 위치에서 이동할 수 있는 범위(bus_charge[distance])를 기준으로 반복문을 실행합니다. 즉, 현재 충전지로 최대 몇 개의 정류장을 이동할 수 있는지를 고려하여 다음 정류장으로 이동합니다.

# 최소 교환 횟수 업데이트 및 가지치기:
# min_cont는 교환 횟수의 최솟값을 추적하기 위한 변수입니다. 현재까지의 교환 횟수(cont)가 min_cont보다 크거나 같으면 더 이상 탐색할 필요가 없으므로 가지치기를 합니다.
